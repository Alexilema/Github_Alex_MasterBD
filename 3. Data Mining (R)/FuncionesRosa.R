...
atipicosAmissing<-function(varaux){
  if (abs(skew(varaux))<1){
    criterio1<-abs((varaux-mean(varaux,na.rm=T))/sd(varaux,na.rm=T))>3 # na.rm=TRUE es para que los valores NA no se cuenten (rm es remove) en el cálculo (por ej, esto hace que la media sea mayor, porque el total de muestras considerado es menor)
  } else {
  ...
  varaux[criterio1&criterio2]<-NA # los valroes que cumplen el criterio será atípicos, así que los cambiamos por NA's... (ver teoría)
  return(list(varaux, sum(criterio1&criterio2, na.rm=T) ))
      # nos devuelve el dt ya sin atípicos, y el número de atípicos, 
      # pues hace un sum de veces que los 2 criterios se han cumplido
}
...
pseudoR2<-function(modelo,dd,nombreVar){
  pred.out.link <- predict(modelo, dd, type = "response") # poner type response para no obtener probabilidades, sino 0 o 1
